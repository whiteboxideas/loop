...
- [x] NS-LOOP-001 Add task-readiness analysis before implementation.
  ...
  - Splitting behavior: If a task is too complex or too broad to implement and validate safely in one loop iteration, split it into smaller independently-checkable backlog tasks instead of starting implementation. Each child task should have a clear goal, bounded scope, validation notes, and an origin reference to the parent task.
  - Chain integration: Readiness analysis can create targeted follow-up backlog tasks such as `needs-info`, `research`, `architecture-question`, `ux-question`, `human-input`, or `split-child`, assigned to personas like `ai:researcher`, `ai:architect`, `ai:ux`, or `human`.
...
- [x] NS-LOOP-002 Add configurable follow-up backlog chains after task completion.
  ...
  - Goal: After completing an implementation task, optionally create one or more follow-up backlog tasks for later loop iterations, such as `review`, `architecture`, `ux`, or `human` tasks.
  ...
  - Acceptance notes: The backlog/current task format and agent prompt should distinguish task type, target user/agent, chain origin, and next-step behavior.
